...
import skrm_word_based as skrm
import argparse
            ...
            for i , dbc in enumerate(tile): 
                ...
                for j in range(real_track_size):
                    ...
                    if nGrams[1024*i+j] == 0 and dbc.racetrack[0][64+j] ^ dbc.racetrack[1][64+j] ^ dbc.racetrack[2][64+j] == 1:
                        insert = insert + 1
                """
                nGrams[1024*i:1024*(i+1)] = dbc.racetrack[0][segment_size:segment_size+real_track_size] ^ dbc.racetrack[1][segment_size:segment_size+real_track_size] ^ dbc.racetrack[2][segment_size:segment_size+real_track_size]
...
def buildLanguageHV(langAM , tile , args):
    ...
    for i in range(len(langLabels)):
        ...
        langHV = computeSumHV(buffer[0:10000] , iM , tile , args)
